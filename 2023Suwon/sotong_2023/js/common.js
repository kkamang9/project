...
(function ($) {
    ...
    $(function () {
        ...
        //여기서부터 코드 작성해주세요
        var $menuBtn = $('.menu_btn .btn'),
            $responsiveMenu = $('.responsive_menu'),
            $responsiveCurtain = $responsiveMenu.find('.menu_curtain'),
            $menuClose = $responsiveMenu.find('.close_btn'),
            $sideMenu = $responsiveMenu.find('.side_menu'),
            $menuOpen = $sideMenu.find('.menu_open'),
            $slideMenu = $sideMenu.find('.depth2');

        // 모바일 lnb 표출
        $menuBtn.on('click',function (){
            $responsiveMenu.addClass('active').find($responsiveCurtain).stop().fadeIn();
        });
        $menuClose.on('click',function (){
            $responsiveMenu.removeClass('active').find($responsiveCurtain).stop().fadeOut();
            $slideMenu.stop().slideUp();
        });

        // 모바일 lnb 아코디언 메뉴
        $menuOpen.on('click',function (){
            $(this).next($slideMenu).stop().slideToggle();
        });
